...
void loop() {
  ...
  if (curTime > startTime + (1000 / fps)) {
    ...
    float cf1 = chainAverageFilter.process(chain);
    chain_filter.Filter(cf1);
    float chain_now = chain_filter.Current();
    ...
    if (!(isinf(top1) || isinf(mid1) || mid1 > 20 || top1 > 20)) {
      ...
      if (top_current < 16 || mid_current < 16 ) {
        //TOP
        if (top_current < 13) {
          if (top_left == 0) {
